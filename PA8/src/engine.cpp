...
bool Engine::Initialize(std::string v, std::string f)
{
  ...
  PhysStruct.add_sphere("Sphere", m_graphics, 1.0);
  //PhysStruct.add_box("BuddhaBase", m_graphics, btVector3(0.0,-10.0,0.0), (btScalar) 1.0);
  return true;
}
...
/*
	for(int iter = 0; iter < (int)PhysStruct.vector_of_shapes.size(); iter++)
	{
		PhysStruct.vector_of_rigid_bodies[iter]->getMotionState()->getWorldTransform(trans);
		trans.getOpenGLMatrix(m);
    		PhysStruct.vector_of_objects[iter]->SetModel(m);
	}
*/
